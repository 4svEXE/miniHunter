function ballMovement(i){
  if (ball[i]) {
    let a = ball[i]
  	...
  	const togAll = (id) => {
  		if ( random(1,100) == 100 || (b.size <= 1 && a.size > 1)) {
  			ball.push(new Ball(b.x, b.y, 1, ball[id].size + b.size/2, '#e4ff03'))
  			ball.splice(id, 1)
   ...
   for (var b of ball) {
    ...
    const collWithPlayer = (id) => {
      if (randBool) a.angle.x = !a.angle.x
      if (randBool) a.angle.y = !a.angle.y

      if (random(1,5) == 1) {
        if (a.type == 'positive') {
          if (random(1,3) == 1) player.size += a.size
        } else {
          if (random(1,2) == 1) player.size -= a.size
        }
    ...
    if (a.left == player.right || a.right == player.left) {
      if (a.bot >= player.top && a.top <= player.bot) {
        collWithPlayer(i)
          if (a.type == 'positive') a.color = 'blue'
      }
    }
    if (a.bot == player.top || a.top == player.bot) {
      if (a.right >= player.left && a.left <= player.right) {
        collWithPlayer(i)
        if (a.type == 'positive') a.color = 'green'
      }
    }
  }
}
